     ...
     return async(dispatch)=>{
        const updateProfile = async()=>{
            const response = await fetch(
                "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyDZDXziHJ-k72uWQKqJVDcch170Y23Hdlg",
                {
                  method: "POST",
                  headers: {
...
export const fetchUserData =(token)=>{
  return async(dispatch)=>{
    const fetchData = async()=>{
      const response = await fetch('https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyDZDXziHJ-k72uWQKqJVDcch170Y23Hdlg',
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
